...
deploy_conjur_master_cluster() {
  ...
  if [[ $CONJUR_DEPLOYMENT == oss ]]; then
    ...
    $cli create secret generic conjur-database-url \
          --from-literal=DATABASE_URL=postgres://postgres:$postgres_password@conjur-postgres/postgres\
          --namespace=$CONJUR_NAMESPACE_NAME
    $cli create secret generic postgres-admin-password \
          --from-literal=POSTGRESQL_ADMIN_PASSWORD=$postgres_password \
          --namespace=$CONJUR_NAMESPACE_NAME
